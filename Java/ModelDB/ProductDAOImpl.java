...
public class ProductDAOImpl implements ProductDAO
{
  ...
  public void creatProduct(Product product) throws SQLException
  {
    try(Connection connection = getConnection(warehouseDB)){
      PreparedStatement statement = connection.prepareStatement("INSERT INTO " + warehouseDB + ".products(products_ID, products_name, products_category, products_productDescription, products_productionDate, products_expirationDate, products_barcode, products_price, products_quantity, products_lowStock, products_unitType ) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
  ...
  @Override
  public Product creat(String ID,String name, int category, String productDescription, MyDate productionDate, MyDate expirationDate,
  ...
  {
   try(Connection connection = getConnection(warehouseDB)){
     PreparedStatement statement = connection.prepareStatement("SELECT * FROM products WHERE products_name LIKE ?");
     ...
     while(resultSet.next()){
       ...
       String productDescription = resultSet.getString("products_productDescription");
  ...
  {
    try(Connection connection = getConnection(warehouseDB)){
      PreparedStatement statement = connection.prepareStatement("UPDATE products SET  products_ID=?,products_name=?,products_category=?, products_productDescription=?, products_productionDate=?, products_expirationDate=?, products_barcode=?, products_price=?, products_quantity=?, products_lowStock=?, products_unitType=? where products_ID=?");
      statement.setString(1,product.getID());
      statement.setString(2, product.getName());
      statement.setInt(3,product.getCategory());
      statement.setString(4,product.getProductDescription());
  ...
  {
    try(Connection connection = getConnection(warehouseDB)){
      PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) AS count FROM "+table+" WHERE products_ID = '"+value+"'");
      ResultSet resultSet = statement.executeQuery();
      resultSet.next();
